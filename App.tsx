...
const AuthContext = createContext<{ isAuthenticated: boolean; login: (user: string, pass: string) => Promise<boolean>; logout: () => void; isAuthLoading: boolean; }>({ isAuthenticated: false, login: async () => false, logout: () => {}, isAuthLoading: true });
...
const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    return <AuthContext.Provider value={{ isAuthenticated, login, logout, isAuthLoading }}>{children}</AuthContext.Provider>;
};
            ...
            <Card className="w-full max-w-sm">
                ...
                <form onSubmit={handleLogin}>
                    ...
                    <button 
                        ...
                        className="w-full bg-primary-dark text-white py-2 rounded-md hover:bg-primary transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                        {isAuthLoading ? 'Initializing...' : t('login')}
                    </button>
                </form>
            </Card>
        </div>
...
const ExpensesPage: React.FC = () => {
    ...
    const handleExport = async () => {
        ...
        // The export function now includes receipts to generate a full statement
        exportExpensesToPdf(expenses, allReceipts, t);
    };
            ...
            <div className="lg:col-span-2">
                <Card>
                    <div className="flex justify-between items-center mb-4">
                        ...
                        <button onClick={handleExport} className="bg-red-500 text-white px-3 py-1 rounded-lg hover:bg-red-600 transition flex items-center gap-2 text-sm"><FiPrinter/> Export PDF</button>
                    </div>
